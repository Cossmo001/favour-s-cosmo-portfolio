  ...
  return (
    <section className="py-24 border-y border-border overflow-hidden bg-background/50">
      ...
      <div className="relative">
        <div className="flex animate-marquee w-max gap-20 sm:gap-32 px-8 items-center">
          {[...brands, ...brands].map((brand, i) => (
            <a
              ...
              className="flex-shrink-0 grayscale-0 sm:grayscale sm:hover:grayscale-0 opacity-80 sm:opacity-40 sm:hover:opacity-100 transition-all duration-300 hover:scale-110"
            >
              <img
                src={brand.image}
                alt={brand.name}
                className={`h-16 sm:h-20 w-auto object-contain ${
                  brand.name === "Fedatify" ? "drop-shadow-[0_0_8px_rgba(0,0,0,0.2)]" : ""
                }`}
              />
            </a>
          ))}
